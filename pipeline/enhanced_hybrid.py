"""
Enhanced Hybrid Pipeline variants — one per WS accuracy improvement technique.
...
Variants:
  T1HybridPipeline — Weighted training (WS labels weighted lower)
  T2HybridPipeline — WS label verification (remove classifier-disagreed labels)
  T3HybridPipeline — Calibrated LFs (Platt scaling)
  T4HybridPipeline — Self-training pseudo-labels (post-loop iterative)
  T5HybridPipeline — Unanimous voting (all LFs must agree)
  T6HybridPipeline — Per-class WS thresholds (class-specific confidence gates)
  ComboHybridPipeline — Combines T1+T2+T3+T6 (all conservative techniques)
"""
    ...
    def run(self, dataset: Dataset) -> PipelineResult:
        ...
        while human_labels_used < cfg.max_human_labels:
            ...
            if train_accuracy >= cfg.ws_accuracy_threshold and human_labels_used >= min_human_for_ws:
                ...
                # AutoWS LFs
                if ws_step_labels < cfg.batch_size:
                    remaining_unlabeled = np.where(~labeled_mask)[0]
                    if len(remaining_unlabeled) > 0:
                        ws = WeakSupervisor(
                            n_classes=dataset.n_classes,
        ...
        result.human_savings_pct = (1 - human_labels_used / cfg.max_human_labels) * 100
    ...
    def run(self, dataset: Dataset) -> PipelineResult:
        ...
        while human_labels_used < cfg.max_human_labels:
            ...
            if step > 0 and step % self.verify_every == 0 and ws_labels_used > 0:
                ...
                labeled_idx_v = np.where(labeled_mask)[0]
                clf_v = create_classifier(cfg.classifier_type, cfg.random_seed)
                clf_v.fit(dataset.X_pool[labeled_idx_v], y_labeled[labeled_idx_v])
    ...
    def run(self, dataset: Dataset) -> PipelineResult:
        ...
        while human_labels_used < cfg.max_human_labels:
            ...
            if step > 0 and step % self.verify_every == 0 and ws_labels_used > 0:
                ...
                wt_mixin.fit_classifier_weighted(clf_v, dataset.X_pool[labeled_idx_v], y_labeled[labeled_idx_v], sw)
                ...
                if ws_step_labels < base_batch:
                    ...
                    if len(remaining_unlabeled) > 0:
                        ...
                        if confident_mask.sum() > 0:
                            ...
                            remaining_quota = max(base_batch - ws_step_labels, 0)
                            if len(ws_indices) > remaining_quota:
                                top_conf = np.argsort(ws_confs)[-remaining_quota:]
                                ws_indices = ws_indices[top_conf]
                                ws_preds = ws_preds[top_conf]
            ...
            if train_accuracy >= cfg.ws_accuracy_threshold and human_labels_used >= min_human_for_ws:
                ...
                if len(remaining_unlabeled) > 0 and ws_step_labels < ws_batch_limit:
                    ...
                    if confident_mask.sum() > 0:
                        ...
                        remaining_quota = max(ws_batch_limit - ws_step_labels, 0)
                        if len(ws_indices) > remaining_quota:
                            top_conf = np.argsort(ws_confs)[-remaining_quota:]
                            ws_indices = ws_indices[top_conf]
                            ws_preds = ws_preds[top_conf]
