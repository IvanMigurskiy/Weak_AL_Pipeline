...
from ..weak_supervision import WeakSupervisor, WeakCertainty, WeakCluster
...
class HybridPipeline:
    ...
    def run(self, dataset: Dataset) -> PipelineResult:
        ...
        # ── WS components ───────────────────────────────────
        use_weak_cert = getattr(cfg, 'use_weak_cert', True)
        use_weak_clust = getattr(cfg, 'use_weak_clust', False)
        weak_cert = WeakCertainty(alpha=cfg.weak_cert_alpha) if use_weak_cert else None
        weak_clust = WeakCluster() if use_weak_clust else None
        query_strategy = QueryStrategy(cfg.query_strategy)
        rng = np.random.default_rng(cfg.random_seed)

        # ── #2 OPTIMIZATION: Create WeakSupervisor ONCE, reuse across steps ──
        ws = WeakSupervisor(
            n_classes=dataset.n_classes,
        ...
        # Track how many labels were added since last WS refit
        _labels_at_last_ws_fit = 0
        ...
        while human_labels_used < cfg.max_human_labels:
            ...
            # ── #3 OPTIMIZATION: Reuse classifier when possible ──
            classifier = create_classifier(cfg.classifier_type, cfg.random_seed)
            classifier.fit(X_train, y_train)
            # Save reference for potential reuse in step 4
            _classifier_for_al = None
            ...
            if train_accuracy >= cfg.ws_accuracy_threshold and human_labels_used >= min_human_for_ws:
                ...
                # 3a2. WeakClust — cluster-based label propagation
                remaining_unlabeled = np.where(~labeled_mask)[0]
                if weak_clust is not None and len(remaining_unlabeled) > 0:
                    weak_clust.fit(dataset.X_pool, labeled_mask, y_labeled)
                    clust_indices, clust_labels = weak_clust.predict(labeled_mask, y_labeled)
                    # Filter to only unlabeled samples
                    if len(clust_indices) > 0:
                        still_unlabeled = ~labeled_mask[clust_indices]
                        clust_indices = clust_indices[still_unlabeled]
                        clust_labels = clust_labels[still_unlabeled]
                    if len(clust_indices) > 0:
                        remaining_quota = max(cfg.batch_size - ws_step_labels, 0)
                        if len(clust_indices) > remaining_quota:
                            clust_indices = clust_indices[:remaining_quota]
                            clust_labels = clust_labels[:remaining_quota]

                        clust_ground_truth = dataset.y_pool[clust_indices]
                        ws_label_correct += int((clust_labels == clust_ground_truth).sum())
                        ws_label_total += len(clust_labels)

                        labeled_mask[clust_indices] = True
                        y_labeled[clust_indices] = clust_labels
                        is_human_label[clust_indices] = False
                        ws_labels_used += len(clust_indices)
                        ws_step_labels += len(clust_indices)

                # 3b. AutoWS LFs — always run (alongside WeakCert)
                # Total WS budget per step = 2× batch_size (WeakCert + AutoWS each get quota)
                remaining_unlabeled = np.where(~labeled_mask)[0]
                if len(remaining_unlabeled) > 0:
                    # ── #2 OPTIMIZATION: Reuse existing WS, only refit if data changed ──
                    n_current_labels = int(labeled_mask.sum())
                    if n_current_labels > _labels_at_last_ws_fit:
                        current_labeled_idx = np.where(labeled_mask)[0]
                        _labeled_texts = [dataset.texts_pool[i] for i in current_labeled_idx]
                        ws.fit(
                            dataset.X_pool[current_labeled_idx],
                            y_labeled[current_labeled_idx],
                            texts=_labeled_texts,
                        )
                        _labels_at_last_ws_fit = n_current_labels
                    _unlabeled_texts = [dataset.texts_pool[i] for i in remaining_unlabeled]
                    weak_labels, weak_confidences = ws.predict(
                        dataset.X_pool[remaining_unlabeled],
                        texts=_unlabeled_texts,
                    )

                    confident_mask = (weak_labels >= 0) & (weak_confidences >= 0.8)
                    if confident_mask.sum() > 0:
                        ...
                        # AutoWS gets its own quota (up to batch_size, on top of WeakCert)
                        autows_quota = max(cfg.batch_size, 0)
            ...
            if len(remaining_unlabeled) > 0 and human_labels_used < cfg.max_human_labels:
                # ── #3 OPTIMIZATION: Reuse classifier if WS didn't add labels ──
                if ws_step_labels == 0 and classifier is not None:
                    # No new WS labels → same labeled set → reuse classifier
                    _classifier_for_al = classifier
                else:
                    labeled_idx = np.where(labeled_mask)[0]
                    _classifier_for_al = create_classifier(cfg.classifier_type, cfg.random_seed)
                    _classifier_for_al.fit(dataset.X_pool[labeled_idx], y_labeled[labeled_idx])
                ...
                query_indices = _select_queries(
                    query_strategy, _classifier_for_al, dataset.X_pool,
        ...
        result.human_savings_pct = (
            (1 - human_labels_used / max(cfg.max_human_labels, 1)) * 100
        )
...
class WSOnlyPipeline:
    ...
    def run(self, dataset: Dataset) -> PipelineResult:
        ...
        # ── #2 OPTIMIZATION: Create WS once, reuse across iterations ──
        ws = WeakSupervisor(
            n_classes=dataset.n_classes,
            lf_confidence_threshold=cfg.lf_confidence_threshold,
            label_model=cfg.label_model,
        ...
        for iteration in range(50):
            ...
            _labeled_texts = [dataset.texts_pool[i] for i in labeled_idx]
            _unlabeled_texts = [dataset.texts_pool[i] for i in unlabeled_idx]
            ws.fit(
                dataset.X_pool[labeled_idx],
                y_labeled[labeled_idx],
                texts=_labeled_texts,
            )
            weak_labels, weak_confidences = ws.predict(
                dataset.X_pool[unlabeled_idx],
                texts=_unlabeled_texts,
            )
...
def _get_effective_use_nb_lf(cfg: PipelineConfig, dataset) -> bool:
    """Return whether NB LF should be used, accounting for encoder compatibility.

    NB (MultinomialNB) requires non-negative features. If the encoder
    produces negative values (e.g., dense embeddings), NB LF is disabled.
    """
    if not cfg.use_nb_lf:
        return False
    # Dense encoders produce negative values → NB can't handle them
    if cfg.encoder_type in ("fasttext_dense", "dense", "hybrid"):
        return False
    return True


def _get_effective_use_keyword_lf(cfg: PipelineConfig, dataset) -> bool:
    """Return whether keyword LF should be used.

    Keyword LF relies on TF-IDF feature names. If feature names are
    unavailable (dense encoders), keyword LF is disabled.
    """
    if not cfg.use_keyword_lf:
        return False
    # Keyword LF needs feature names from sparse encoders
    if cfg.encoder_type in ("fasttext_dense", "dense"):
        return False
    return True
