...
def main():
    ...
    parser.add_argument(
        "--classifier", choices=["rf", "lr", "svm"], default="lr",
    )
    ...
    parser.add_argument(
        "--encoder",
        choices=["tfidf", "bm25", "fasttext_sparse", "fasttext_dense", "splade", "dense", "hybrid"],
        default=None,
        help="Text encoder type (default: tfidf)",
    )
    parser.add_argument(
        "--encoder-comparison",
        action="store_true",
        help="Run encoder ablation study (compare all available encoders)",
    )
    ...
    # Encoder comparison mode
    if args.encoder_comparison:
        from .experiments.encoder_comparison import run_encoder_comparison
        for ds_name in datasets_to_run:
            try:
                ...
                print(f"ERROR on {ds_name} encoder comparison: {e}")
                import traceback
                traceback.print_exc()
